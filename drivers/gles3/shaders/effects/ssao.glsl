...
// WebGL2.0 Storage buffer is limited, We use const value here.(different from forward+)
...
#ifdef ADAPTIVE
// not support now, will not use these texture
...
// will not use. now we use normal texture.
...
void main() {
	/* Note */
	// main differences from forward+:
	// 1. forawrd+ use view space linear texture, here we use ndc depth texture and convert it into view space
	// 2. some differences in things related to normal texture(e.g. uv coord)
	// 3. not use constant buffer here
