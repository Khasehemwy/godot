...
SSAO::SSAO() {
	...
	{
		// get rotation_matrices.
		// not use actually. we use const array in shader now.
...
void SSAO::generate_ssao(SSAORenderBufferProperty &p_ssao_buffers, Buffers &p_buffers, int p_view_index, const Projection &p_projection, const SSAOSettings &p_settings, GLuint
	...
	// we need save origin states, restore them later
	...
	/* FIRST PASS*/
	/* in forward+ is view linear depth generate pass*/
	/* in compatibility is normal generate pass*/
	...
	//Gather
	{
		...
		// size_multiplier is different from forward+
		ssao.gather_push_constant.size_multiplier = ssao_half_size ? 4 : (2 - k_is_frambuffer_same_size);

		// version_bind_shader will auto call glUseProgram().
		...
		for (int i = 0; i < 4; i++) {
			...
			// output texture(deinterleaved.color) is Texture2DArray, We output to specific layer
	...
	//
	{
		...
		for (int pass = 0; pass < blur_passes; pass++) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (pass % 2 == 0) {
					// use deinterleaved.color, output to deinterleaved_pong.color
					glActiveTexture(GL_TEXTURE0);
					...
					// use deinterleaved_pong.color, output to deinterleaved.color
					glActiveTexture(GL_TEXTURE0);
